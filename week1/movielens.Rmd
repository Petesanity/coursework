...
#
ggplot(ratings) + geom_histogram(aes(x = rating)) + scale_y_continuous(label = comma) 
...
ratings %>% group_by(movie_id) %>% summarize(num_rating = n())%>%
  ggplot(aes(x = movie_id, y = num_rating)) + geom_point() + scale_x_log10(label = comma)
...
movie_rank <- ratings %>% group_by(movie_id) %>% summarize(movierank = n()) %>% arrange(desc(movierank)) %>% mutate(cdf = cumsum(movierank)/sum(movierank))

#plot the CDF of movie popularity
ratings %>% group_by(movie_id) %>% summarize(movierank = n()) %>% arrange((movierank)) %>% mutate(cdf = cumsum(movierank)/sum(movierank)) %>%
ggplot() + geom_point( aes(x = movierank, y= cdf))
...
ratings %>% group_by(user_id) %>% summarize(num_ratings = n()) %>% ggplot(aes(x = user_id, y = num_ratings)) + geom_point() + scale_x_log10(label = comma)
